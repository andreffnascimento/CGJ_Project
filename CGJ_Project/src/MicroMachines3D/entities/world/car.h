...
public:
		...
		: SceneEntity(scene->createEntity("Car"))
	{
		...
		_addCarHeadlight(scene, group, "frontLeft", 1.4f);
		_addCarHeadlight(scene, group, "frontRight", -1.4f);
		...
		rigidbody.setMaxAngularVelocity(200.0f);
...
private:
	void _addCarHeadlight(Scene* scene, GroupComponent& group, const char* headlightId, float sideMod)
	{
		...
		Transform::translate(headlight, Coords3f({ 1.0f * sideMod, 0.6f, 3.0f }));
